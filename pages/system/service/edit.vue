<template>
  <view class="container">
    <view class="uni-header">
      <view class="uni-title">编辑服务</view>
    </view>
    ...
    methods: {
      async getDetail() {
        try {
          ...
          const res = await db.collection('pet-services').doc(this.id).get()
      ...
      submit() {
        this.$refs.form.validate().then(async (res) => {
          try {
            const db = uniCloud.database()
            await db.collection('pet-services').doc(this.id).update({
